...
from planning import bellman_ford_with_midwaypoints
...
class PlanningState:
    ...
    w_s_repr: jnp.ndarray
    w_g_repr: jnp.ndarray
    shortest_w_pdists: jnp.ndarray
    mid_waypoints: jnp.ndarray
...
def main(args):
    ...
    planning_state = PlanningState(
        ...
        w_s_repr=jnp.zeros((args.num_candidates, args.repr_dim), dtype=jnp.float32),
        w_g_repr=jnp.zeros((args.num_candidates, args.repr_dim), dtype=jnp.float32),
        shortest_w_pdists=jnp.zeros((args.num_candidates, args.num_candidates), dtype=jnp.float32),
        mid_waypoints=jnp.zeros((args.num_candidates, args.num_candidates), dtype=jnp.int32),
    )
    ...
    def search_waypoint(training_state, env_state, planning_state):
        ...
        # sw_dists = -energy_fn(s_repr[:, None], planning_state.waypoint_obs)  # (B, N)
        # wg_dists = -energy_fn(planning_state.w_repr[None], projected_g_repr[:, None])  # (B, N)
        # sg_dist = -energy_fn(s_repr, projected_g_repr)  # (B, )

        state_xy = state[:, args.goal_start_idx:args.goal_end_idx]
        ...
        first_w_idxs = jnp.argmin(jnp.min(search_dists, axis=2), axis=1)  # (B, )
        last_w_idxs = jnp.argmin(jnp.min(search_dists, axis=1), axis=1)  # (B, )
        w_idxs = planning_state.mid_waypoints[first_w_idxs, last_w_idxs]
    ...
    def update_planning_state(transitions, training_state, planning_state):
        ...
        shortest_w_pdists, _, mid_waypoints, _ = jax.vmap(bellman_ford_with_midwaypoints, in_axes=(None, 0))(
            w_pdists, jnp.arange(waypoint_obs.shape[0]))
