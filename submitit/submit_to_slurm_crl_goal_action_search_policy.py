        ...
        for env_id in ["ant_randomized_init_u_maze"]:
            for train_planner in ["no-train_planner"]:
                for eval_planner in ["no-eval_planner"]:
                        for resubs in ["resubs", "no-resubs"]:  # (chongyiz): resubs is important for this algorithm!!
                            for sym_infonce in ["sym_infonce"]:
                                for log1msoftmax in ["no-log1msoftmax"]:
                                    for quasimetric_energy_type in ["none"]:
                                        for repr_dim in [16, 128]:
                                            for total_env_steps in [10_000_000]:
                                                for seed in [0]:
                                                    exp_name = f"phi_sa_psi_phi_gag_sg_forward_infonce_planner={train_planner}_{eval_planner}_{resubs}_quasimetric={quasimetric_energy_type}_repr_dim={repr_dim}_{sym_infonce}_{log1msoftmax}"
