        ...
        for env_id in ["ant_randomized_init_u_maze"]:
            for train_planner in ["train_planner"]:
                for eval_planner in ["eval_planner"]:
                    for planner_use_repr in ["planner_use_repr"]:
                        for max_edge_dist in [3.0, 5.0]:  # use 3.0 for no-planner_use_repr
                            for resubs in ["resubs", "no-resubs"]:
                                for sym_infonce in ["sym_infonce"]:
                                    for log1msoftmax in ["no-log1msoftmax"]:
                                        for total_env_steps in [10_000_000]:
                                            for seed in [1, 2]:
                                                exp_name = f"crl_search_policy_planner={train_planner}_{eval_planner}_{planner_use_repr}_max_edge_dist={max_edge_dist}_{resubs}_{sym_infonce}_{log1msoftmax}_dijkstra_0.25_waypoints_qv_separate_encoders_v_use_xy"
                                                log_dir = os.path.expanduser(
                                                    f"{log_root_dir}/exp_logs/jax_gcrl_logs/crl_search_policy_planner/{exp_name}/{seed}")
                                                ...
                                                cmds = f"""
                                                    ...
                                                    rm -rf {log_dir};
                                                    mkdir -p {log_dir};
                                                    python $PROJECT_DIR/clean_JaxGCRL/train_crl_search_policy_planner_jax_brax.py \
                                                        --track \
                                                        --{resubs} \
                                                        --{sym_infonce} \
                                                        --{log1msoftmax} \
