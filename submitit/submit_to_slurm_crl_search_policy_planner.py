        ...
        for env_id in ["ant_randomized_init_u_maze"]:
            for train_planner in ["train_planner"]:
                for eval_planner in ["eval_planner"]:
                    for resubs in ["resubs"]:
                        for sym_infonce in ["no-sym_infonce"]:
                            for log1msoftmax in ["no-log1msoftmax"]:
                                for total_env_steps in [10_000_000]:
                                    for seed in [0]:
                                        exp_name = f"crl_search_policy_planner={train_planner}_{eval_planner}_forward_infonce_{resubs}_{sym_infonce}_{log1msoftmax}"
